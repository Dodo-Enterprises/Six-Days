...
class Weapon:
    ...
    def __init__(self, cost, job, wpn_type, damage, effect):
...
class Armor:
    """Weapon data type that holds all the relevant information for all weapon items."""
    def __init__(self, cost, job, phy_neg, magic_neg, armor_type, armor_piece, effect):
        ...
        armor_piece -- the type of armor piece it is (Helmet, breastplate, and grives)
...
class Spell:
    ...
    def __init__(self, cost, mana_cost, damage, is_AOE, effect):
...
class Potion:
    ...
    def __init__(self, cost, effect, potency):
        """Creates the Potion instance.

        Keyword arguments:
        cost -- the value of this item in currency
        effect -- the specific effect this potion causes
        potency -- the amount of effect the effect has
        """
        assert (isinstance(cost, int), f"Cost expected to be Integer type, got: {type(cost)}")
        assert (isinstance(effect, Effects), f"Effect expected to be Effects type, got: {type(effect)}")
        assert (isinstance(potency, int), f"Potency expected to be Integer type, got: {type(potency)}")
        self.cost = cost
        self.effect = effect
